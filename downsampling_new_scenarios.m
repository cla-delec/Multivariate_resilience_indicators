...
res_tested=[1, 2, 4, 8, 12]; 
n_lengths=size(res_tested,2);
...
results_taus_agg_zsnans_tp_multi=zeros(n_indic_multi,n_scenarios_multi,number_rep,n_lengths); 
results_taus_agg_zsnans_tn_multi=zeros(n_indic_multi,n_scenarios_multi,number_rep,n_lengths); 
results_taus_agg_zsnans_tp_uni=zeros(n_indic_uni,n_scenarios_uni,number_rep,n_lengths); 
results_taus_agg_zsnans_tn_uni=zeros(n_indic_uni,n_scenarios_uni,number_rep,n_lengths); 

for ind_cur_length=1:n_lengths
    
    cur_res=res_tested(ind_cur_length);
    ...
    for cur_rep=1:number_rep
        ...
        for cur_scenario_multi=1:n_scenarios_multi
            ...
            cur_data_processed_tp_subsampled=cur_data_processed_tp(1:cur_res:length_tot_aggregated,:);
            
            ews=generic_ews(cur_data_processed_tp_subsampled,'indicators',{'degfinger', 'mafac', 'mafeig', 'mean_ar','max_ar','mafvar','max_var','mean_var','pcavar','explvar','max_abscorr','max_cov'},'datacolumn',[],'silent',true,'nanflag','omitnan'); %,'ebisuzaki',100
            cur_taus=ews.taus;
            results_taus_agg_zsnans_tp_multi(:,cur_scenario_multi,cur_rep,ind_cur_length)=cur_taus;
            ...
            cur_data_processed_tn_subsampled=cur_data_processed_tn(1:cur_res:length_tot_aggregated,:);


            ews=generic_ews(cur_data_processed_tn_subsampled,'indicators',{'degfinger', 'mafac', 'mafeig', 'mean_ar','max_ar','mafvar','max_var','mean_var','pcavar','explvar','max_abscorr','max_cov'},'datacolumn',[],'silent',true,'nanflag','omitnan'); %,'ebisuzaki',100
            cur_taus=ews.taus;
            results_taus_agg_zsnans_tn_multi(:,cur_scenario_multi,cur_rep,ind_cur_length)=cur_taus;
        end
        ...
        for cur_scenario_uni=1:n_scenarios_uni
            ...
            cur_data_tp_uni_processed_subsampled=cur_data_tp_uni_processed_tp(1:cur_res:length_tot_aggregated,:);
            
            ews=generic_ews(cur_data_tp_uni_processed_subsampled,'indicators',{'var','ar'},'silent',true,'nanflag','omitnan'); %,'ebisuzaki',100
            cur_taus=ews.taus;
            results_taus_agg_zsnans_tp_uni(:,cur_scenario_uni,cur_rep,ind_cur_length)=cur_taus;
            ...
            cur_data_tn_uni_processed_subsampled=cur_data_tn_uni_processed_tn(1:cur_res:length_tot_aggregated,:);
            ews=generic_ews(cur_data_tn_uni_processed_subsampled,'indicators',{'std','ar'},'silent',true,'nanflag','omitnan'); %,'ebisuzaki',100
            cur_taus=ews.taus;
            results_taus_agg_zsnans_tn_uni(:,cur_scenario_uni,cur_rep,ind_cur_length)=cur_taus;
        end
...
save("data/results_extrares_feedingpref80_downsampling100-429-5res_alltaus_siminf_length3000_100rep_tp_k0.35-0.50_R00.7-1_tn_k0.40_R00.8_rateimport0.15_agg.mat",'final_results');

%% load results

final_result=load("data/results_extrares_feedingpref80_downsampling100-429-5res_alltaus_siminf_length3000_100rep_tp_k0.35-0.50_R00.7-1_tn_k0.40_R00.8_rateimport0.15_agg.mat");
...
results_multi=zeros(n_indic_multi,n_lengths,n_scenarios_multi); 
result_uni=zeros(n_indic_uni,n_lengths,n_scenarios_uni);
...
for ind_cur_length=1:n_lengths
    for cur_scenario_multi=1:n_scenarios_multi
        for cur_indic_multi=1:n_indic_multi
            res_tp_scen_indic_length=squeeze(results_taus_agg_zsnans_tp_multi(cur_indic_multi,cur_scenario_multi,:,ind_cur_length));
            res_tn_scen_indic_length=squeeze(results_taus_agg_zsnans_tn_multi(cur_indic_multi,cur_scenario_multi,:,ind_cur_length));

            [X_cur,Y_cur,T,results_multi(cur_indic_multi,ind_cur_length,cur_scenario_multi)]=perfcurve(label_tot,[res_tp_scen_indic_length.',res_tn_scen_indic_length.'],'epidemics','ProcessNaN','addtofalse'); 
        end
    end
    
    for cur_scenario_uni=1:n_scenarios_uni    
        for cur_indic_uni=1:n_indic_uni
            res_tp_scen_indic_length=squeeze(results_taus_agg_zsnans_tp_uni(cur_indic_uni,cur_scenario_uni,:,ind_cur_length));
            res_tn_scen_indic_length=squeeze(results_taus_agg_zsnans_tn_uni(cur_indic_uni,cur_scenario_uni,:,ind_cur_length));

            [X_cur,Y_cur,T,result_uni(cur_indic_uni,ind_cur_length,cur_scenario_uni)]=perfcurve(label_tot,[res_tp_scen_indic_length.',res_tn_scen_indic_length.'],'epidemics','ProcessNaN','addtofalse'); 
        end
    end
end
...
save("data/results_aucs_extrares_downsampling100-429-5res_alltaus_siminf_feedingpref80_length3000_10rep_tp_k0.35-0.50_R00.7-1_tn_k0.40_R00.8_rateimport0.15_agg.mat",'final_aucs');

%% LOAD RESULTS

final_res=load("data/results_aucs_extrares_downsampling100-429-5res_alltaus_siminf_feedingpref80_length3000_10rep_tp_k0.35-0.50_R00.7-1_tn_k0.40_R00.8_rateimport0.15_agg.mat");
...
for k=1:n_scenarios_multi
    ...
    xlabel('Resolution'); ylabel('AUC');
    for i=1:n_indic_autocor
        id_indic_autocor=id_autocor_based(i);
        hold on
        plot(res_tested,results_multi(id_indic_autocor,:,k),'o-','LineWidth',2);
    ...
    for j=1:size(id_uni_cur_multi,1)
        ...
        plot(res_tested,result_uni(2,:,id_cur_uni),'o--','LineWidth',2,'Color',cols(id_cur_uni));
    end
    ...
    for i=1:n_indic_var
        ...
        hold on
        plot(res_tested,results_multi(id_indic_var,:,k),'o-','LineWidth',2);
    end
    xticklabels({'Weekly', 'Biweekly', ' ', 'Monthly'})   
    for j=1:size(id_uni_cur_multi,1)
        id_cur_uni=id_uni_cur_multi(j);
        plot(res_tested,result_uni(1,:,j),'o--','LineWidth',2,'Color',cols(id_cur_uni));
    end
...
xticks(1:12)
xticklabels({'Weekly','Biweekly','','Monthly','','','', 'Bimonthly','','','', 'Trimonthly'})
...
for j=1:n_selected_uni
    ...
    plot(res_tested,result_uni(1,:,j),'o--','LineWidth',2,'Color',cols_uni(j));
end
...
for j=1:n_scenarios_uni
    plot(res_tested,result_uni(2,:,j),linetypes(j),'LineWidth',2,'Color',cols(j));
end
hold off
...
for j=1:n_scenarios_uni
    ...
    plot(res_tested,result_uni(1,:,j),linetypes(j),'LineWidth',2,'Color',cols(j));
end
hold off
legend(name_scenarios_uni2,'Location','southoutside')
xticks(1:12)
...
for i=1:n_indic_autocor
    id_indic_autocor=id_autocor_based(i);
    hold on
    plot(res_tested,results_multi(id_indic_autocor,:,k),'^-','LineWidth',2);
...
%endtested,results_multi(id_indic_autocor,:,k),'^-','LineWidth',2);
...
for j=1:size(id_uni_cur_multi,1)
    id_cur_uni=id_uni_cur_multi(j);
    plot(res_tested,result_uni(2,:,id_cur_uni),'^--','LineWidth',2,'Color',cols(id_cur_uni));
end
%legend([indicators_multi(id_autocor_based) name_scenarios_uni2(id_uni_cur_multi)],'Location','southoutside')
xlim([0 5])
ylim([0.5 1])
...
plot(res_tested,mean(results_multi(id_autocor_based,:,k),1),'-o','LineWidth',2,'Color',"#0072BD",'MarkerSize',7);
plot(res_tested,mean(results_multi(id_var_based2,:,k),1),'-s','LineWidth',2,'Color',"#0072BD",'MarkerSize',10,'MarkerFaceColor',"#0072BD");
xticklabels({'Weekly','', 'Biweekly', ' ','','', 'Monthly'})
yticks(0.5:0.1:1)
...
id_uni_cur_multi=find(ismember(scenarios_uni,scenarios_multi(k,:)));
for j=1:size(id_uni_cur_multi,1)
    plot(res_tested,result_uni(2,:,j),'--o','LineWidth',2,'Color', cols(j),'MarkerSize',7);
end

%plot var uni
for j=1:size(id_uni_cur_multi,1)
    plot(res_tested,result_uni(1,:,j),'--s','LineWidth',2,'Color', cols(j),'MarkerSize',10,'MarkerFaceColor',cols(j));
end
